...
const PostCard = ({ post }) => {
  ...
  const buttonColor = category === 'stage-2' ? 'button_stage_2' : 'button_stage_1';
  return (
    <div className="glass_card shadow-lg rounded-lg p-0 pb-12 mb-8">
      ...
      <div className="text-center">
        <Link href={`/post/${post.slug}`}>
          <span className={`transition duration-500 ease transform hover:-translate-y-1 inline-block ${buttonColor} text-lg font-medium rounded-full text-white px-8 py-3 cursor-pointer`}>Continue Reading</span>
        </Link>
      </div>
    </div>
  );
